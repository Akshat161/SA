...
int main() {
 ...
 while(completed != x) {
    ...
    cout<<endl<<endl;
     ...
     cout<<"Process\t"<<"PT\t"<<"AT\t"<<"BT\t"<<"ST\t"<<"CT\t"<<"TAT\t"<<"WT\t"<<"RT\t"<<"\n"<<endl;


    for(int i = 0; i < x; i++) {
        cout<<p[i].pid<<"\t"<<p[i].priority<<"\t"<<p[i].arrival_time<<"\t"<<p[i].burst_time<<"\t"<<p[i].start_time<<"\t"<<p[i].completion_time<<"\t"<<p[i].turnaround_time<<"\t"<<p[i].waiting_time<<"\t"<<p[i].response_time<<"\t"<<"\n"<<endl;
    }
